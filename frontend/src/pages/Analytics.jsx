...
const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
        <div className="bg-[#1a1d2e] text-white px-3 py-2 rounded-lg shadow-xl text-xs border border-white/10">
...
const ChartCard = ({ title, icon: Icon, children, delay = 0, className = '' }) => (
    <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        transition={{ delay, duration: 0.4, ease: 'easeOut' }}
        className={`bg-[#1a1d2e] rounded-xl border border-white/[0.06] shadow-lg shadow-black/10 p-5 ${className}`}
    >
...
const Analytics = () => {
    ...
    const getDateRange = useCallback(() => {
        ...
        if (period === 'month') {
            const start = new Date(end); start.setMonth(start.getMonth() - 1); start.setHours(0, 0, 0, 0);
            return { start, end };
        }
        if (period === 'custom') {
    ...
    const filteredExpenses = useMemo(() => {
        ...
        return expenses.filter(e => { const d = new Date(e.createdAt); return d >= start && d <= end; });
    }, [expenses, getDateRange]);

    const filteredPayments = useMemo(() => {
        const { start, end } = getDateRange();
        return payments.filter(p => { const d = new Date(p.createdAt); return d >= start && d <= end; });
    ...
    const monthlyData = useMemo(() => {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const expTotal = expenses.filter(e => { const dt = new Date(e.createdAt); return dt.getMonth() === month && dt.getFullYear() === year; }).reduce((s, e) => s + e.amount, 0);
            const payTotal = payments.filter(p => { const dt = new Date(p.createdAt); return dt.getMonth() === month && dt.getFullYear() === year; }).reduce((s, p) => s + p.amount, 0);
    ...
    const weeklyData = useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const total = expenses.filter(e => { const dt = new Date(e.createdAt); return dt >= dayStart && dt < dayEnd; }).reduce((s, e) => s + e.amount, 0);
    ...
    const handlePeriod = (key) => { setPeriod(key); };
                ...
                <div className="bg-[#13151f] border-b border-white/[0.04] px-4 py-3 sm:px-6">
                    <div className="max-w-6xl mx-auto flex flex-wrap items-center gap-3">
                            ...
                            {[
                                ...
                                <button key={tab.key}
                                    ...
                                    className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all duration-200 ${period === tab.key
                                        ? 'bg-white/[0.1] text-indigo-300 shadow-sm'
                            ...
                            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <input type="date" value={dateFrom} max={dateTo}
                                        ...
                                        className="px-2 py-1.5 text-xs border border-white/[0.08] rounded-lg bg-[#1e2235] text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
                                </div>
                                <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-slate-500 ml-auto">
                            ...
                            <span className="font-medium text-indigo-400">{filteredExpenses.length + filteredPayments.length} records</span>
                        </div>
                    </div>
                </div>
                ...
                <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <ChartCard title="Monthly Overview" icon={Activity} delay={0.2} className="lg:col-span-2">
                            {monthlyData.some(d => d.expenses > 0 || d.payments > 0) ? (
                                ...
                                <div className="h-[260px] flex items-center justify-center text-slate-600 text-sm">No data for this period</div>
                            )}
                        </ChartCard>
                                    ...
                                    <div className="flex items-center justify-center gap-5 mt-2">
                                        {methodData.map((m, i) => (
                                            <div key={i} className="flex items-center gap-2 text-xs">
                                                ...
                                                <span className="text-slate-500 font-bold">₹{fmt(m.value)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ) : (
                                <div className="h-[220px] flex items-center justify-center text-slate-600 text-sm">No payment data yet</div>
                            )}
                        </ChartCard>
                    </div>

                    {/* ─── Row: Weekly Spending + Comparison ─── */}
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    ...
                                    <BarChart data={weeklyData} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
                                        ...
                                        <Bar dataKey="amount" name="Spent" radius={[6, 6, 0, 0]} maxBarSize={36}>
                                            {weeklyData.map((entry, i) => (
                                                <Cell key={i} fill={i === weeklyData.length - 1 ? COLORS.indigo : '#3d4260'} />
                                            ))}
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="h-[220px] flex items-center justify-center text-slate-600 text-sm">No expenses this week</div>
                            )}
                        </ChartCard>
                        ...
                        <ChartCard title="Expenses vs Payments" icon={BarChart3} delay={0.4}>
                            {monthlyData.some(d => d.expenses > 0 || d.payments > 0) ? (
                                ...
                                <div className="h-[220px] flex items-center justify-center text-slate-600 text-sm">No data for this period</div>
                            )}
                        </ChartCard>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
                            className="bg-[#1a1d2e] rounded-xl border border-white/[0.06] shadow-lg p-5 flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                                <Banknote className="h-6 w-6 text-emerald-400" />
                            </div>
                            <div>
                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Cash Payments</p>
                                <p className="text-xl font-extrabold text-white">₹{fmt(cashPayments.reduce((s, p) => s + p.amount, 0))}</p>
                                <p className="text-xs text-slate-600">{cashPayments.length} transaction{cashPayments.length !== 1 ? 's' : ''}</p>
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                            ...
                            <div>
                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">UPI Payments</p>
                                <p className="text-xl font-extrabold text-white">₹{fmt(upiPayments.reduce((s, p) => s + p.amount, 0))}</p>
                                <p className="text-xs text-slate-600">{upiPayments.length} transaction{upiPayments.length !== 1 ? 's' : ''}</p>
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
                            ...
                            <div>
                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Active Groups</p>
                                <p className="text-xl font-extrabold text-white">
                                    {new Set([...filteredExpenses.map(e => e.groupId), ...filteredPayments.map(p => p.groupId)]).size}
                                </p>
                                <p className="text-xs text-slate-600">Groups with transactions</p>
                            </div>
                        </motion.div>
                    </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                        ...
                        <div className="bg-[#1a1d2e] rounded-xl border border-white/[0.06] shadow-lg overflow-hidden">
                            {recentActivity.length === 0 ? (
                                <div className="p-8 text-center text-slate-600 text-sm">No activity for this period</div>
                            ) : (
                                <ul className="divide-y divide-white/[0.04]">
                                    {recentActivity.map((item, i) => (
                                        <motion.li key={item.id + item.type}
                                            ...
                                            className="px-5 py-3.5 flex items-center justify-between hover:bg-white/[0.03] transition-colors">
                                            ...
                                            <span className={`text-sm font-bold flex-shrink-0 ml-3 ${item.type === 'expense' ? 'text-white' : 'text-emerald-400'}`}>
